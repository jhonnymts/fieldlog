  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      ...
      <div className="bg-card border border-border rounded-xl p-5 mb-6">
        ...
        <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">{project.location && <span className="flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5" /> {project.location}</span>}{project.project_number && <span className="flex items-center gap-1.5"><Hash className="h-3.5 w-3.5" /> {project.project_number}</span>}{project.start_date && <span className="flex items-center gap-1.5"><Calendar className="h-3.5 w-3.5" /> {format(new Date(project.start_date + 'T12:00:00'), 'MMM d, yyyy')}</span>}</div>
      </div>
