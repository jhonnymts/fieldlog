      ...
      <div className="flex items-center gap-3 p-3 cursor-pointer active:bg-secondary/50" onClick={() => setExpanded(!expanded)}>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            <span className="text-xs font-mono text-primary font-medium">{asset.asset_id}</span>
          ...
          {asset.tags && asset.tags.length > 0 && (
            <div className="mt-1">
              <TagInput tags={asset.tags} onChange={() => {}} readOnly size="xs" />
            </div>
          )}
        </div>
        <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
          {quickStatuses.map((s) => (
            <button key={s.label} onClick={() => onUpdate && onUpdate({ status: s.label })}
              className={`text-xs px-2 py-0.5 rounded-full border font-medium transition-all ${asset.status === s.label ? s.color + ' ring-1 ring-current' : 'bg-transparent text-muted-foreground border-border'}`}>
              {s.label === 'In Progress' ? '⏳' : s.label === 'Open Issue' ? '🔶' : '✅'}
            </button>
          ))}
        </div>
      ...
      {expanded && (
        <div className="border-t border-border p-3 space-y-3 bg-secondary/30">
          <div>
            ...
            <Select value={asset.status} onValueChange={(v) => onUpdate && onUpdate({ status: v })}>
          ...
          <div>
            ...
            <Textarea value={asset.notes || ''} onChange={(e) => onUpdate && onUpdate({ notes: e.target.value })}
              placeholder="Add notes..." className="bg-secondary border-border mt-1 min-h-[60px] resize-none" />
          </div>
          <div>
            <label className="text-xs text-muted-foreground uppercase tracking-wider block mb-1">Tags</label>
            <TagInput tags={asset.tags || []} onChange={(tags) => onUpdate && onUpdate({ tags })} />
          </div>
          {onDelete && (
