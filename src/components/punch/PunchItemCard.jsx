...
export default function PunchItemCard({ item, onUpdate, onDelete }) {
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            ...
            <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${style.pill}`}>
              {item.status}
            </span>
          </div>
          <p className={`text-sm text-foreground ${isClosed ? 'line-through text-muted-foreground' : ''}`}>
            {item.description}
          </p>
